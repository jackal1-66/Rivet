  ...
  public:
    ...
    void analyze(const Event& event) {
      ...
      if(beamp.first == 2212 && beamp.second == 2212){
        //pp cycle
        if(fuzzyEquals(sqrtS()/GeV,5020)){ //5.02 TeV
         ...
         foreach (const Particle& p, ufs.particles()) {  // 7 TeV
            if(p.fromBottom())
                continue;
            else{    
                 if(p.absrap() < 0.5){
    ...
    void finalize() {
      /*At this point cross sections consider both particles and antiparticles, hence a factor 2 is added 
      in the histos normalization in order to account for this (as done in the paper)*/
