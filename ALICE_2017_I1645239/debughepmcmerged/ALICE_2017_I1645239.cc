...
namespace Rivet {
  ...
  public:
         ...
         foreach (const Particle& p, ufs.particles()) {
            if(p.fromBottom())
                ...
                {    
                 if(p.absrap() < 0.5){
                     if(p.abspid() == 421){
                         _h_D0->fill(p.pT()/GeV, 1); 
                         _h_D0int->fill(0,1);}
                     else if(p.abspid() == 4122){
                         _h_Lc->fill(p.pT()/GeV, 1);
			 _h_Lcdummy->fill(p.pT()/GeV, 1);
                         _h_Lcint->fill(0,1);}
                     }   
                }    
        }
    ...
    void finalize() {
      if(bo1 == true) scale(_h_D0, crossSection()/(microbarn*2*numEvents()/3)); // norm to cross section
      if(bo1 == true) scale(_h_D0int, crossSection()/(microbarn*2*numEvents()/3)); // norm to cross section
      if(bo1 == true) scale(_h_Lc, crossSection()/(microbarn*2*numEvents()/3)); // norm to cross section
      if(bo1 == true) scale(_h_Lcdummy, crossSection()/(microbarn*2*numEvents()/3)); // norm to cross section
      if(bo3 == true) scale(_h_LcPbdummy, crossSection()/(microbarn*2*numEvents()/3)); // norm to cross section
      if(bo1 == true) scale(_h_Lcint, crossSection()/(microbarn*2*numEvents()/3)); //norm to cross section
      if(bo3 == true) scale(_h_D0Pb, crossSection()/(microbarn*2*numEvents()/3)); // norm to cross section
      if(bo3 == true) scale(_h_D0intPb, crossSection()/(microbarn*2*numEvents()/3)); // norm to cross section
      if(bo3 == true) scale(_h_LcPb, crossSection()/(microbarn*2*numEvents()/3)); // norm to cross section
      if(bo3 == true) scale(_h_LcintPb, crossSection()/(microbarn*2*numEvents()/3)); //norm to cross section
      ...
      if(bo2 == true) scale(_h_LcR, 208*crossSection()/(microbarn*2*numEvents()/3)); // norm to cross section
      if(bo3 == true) scale(_h_LcRPb, crossSection()/(microbarn*2*numEvents()/3)); // norm to cross section
